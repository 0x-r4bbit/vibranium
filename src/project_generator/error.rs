    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ProjectGenerationError::ProjectPathNotFound => {
                write!(f, "Couldn't find directory for given project path")
            }
            ProjectGenerationError::VibraniumDirectoryNotFound => write!(
                f,
                "Not a Vibranium project. Couldn't find .vibranium directory"
            ),
            ProjectGenerationError::Io(error) => write!(f, "{}", error),
            ProjectGenerationError::Serialization(error) => {
                write!(f, "Couldn't serialize data: {}", error)
            }
...
impl From<config::error::ConfigError> for ProjectGenerationError {
    fn from(error: config::error::ConfigError) -> Self {
        match error {
            config::error::ConfigError::Deserialization(_) => {
                ProjectGenerationError::InvalidConfig(error)
            }
