...
impl Vibranium {
    ...
    ) -> Result<ExitStatus, blockchain::error::NodeError> {
    ...
    pub fn reset_project(
        &self,
        reset_options: project_generator::ResetOptions,
    ) -> Result<(), project_generator::error::ProjectGenerationError> {
        let generator = project_generator::ProjectGenerator::new(&self.config);
        generator
    ...
    ) -> Result<Output, compiler::error::CompilerError> {
        ...
        generator
            ...
            .and_then(|_| {
                compiler.compile(config).map(|process| {
                    process
                        .wait_with_output()
                        .map_err(compiler::error::CompilerError::Io)
                })
            })
            .and_then(|output| output)
            .and_then(|output| {
                if !output.status.success() {
                    Err(compiler::error::CompilerError::Other(
    ...
    pub fn get_blockchain_connector(
        &self,
    ) -> Result<
        (
        ...
        blockchain::error::ConnectionError,
    > {
        let generator = project_generator::ProjectGenerator::new(&self.config);
        ...
        generator
            ...
            .and_then(|_| {
                let project_config = self
                    ...
                    .read()
                    .map_err(|err| blockchain::error::ConnectionError::Other(err.to_string()))?;
                let blockchain_config = project_config
                    .blockchain
                    .ok_or(blockchain::error::ConnectionError::MissingConnectorConfig)?;
                let connector_config = blockchain_config
                    .connector
                    .ok_or(blockchain::error::ConnectionError::MissingConnectorConfig)?;
    ...
    ) -> Result<deployment::DeployedContracts, deployment::error::DeploymentError> {
        let (_eloop, connector) = self
            .get_blockchain_connector()
            .map_err(deployment::error::DeploymentError::Connection)?;
    ...
    pub fn get_tracking_data(
        &self,
    ) -> Result<
        Option<deployment::tracker::SmartContractTrackingData>,
        deployment::error::DeploymentTrackingError,
    > {
        let (_eloop, connector) = self.get_blockchain_connector()?;
        let tracker = deployment::tracker::DeploymentTracker::new(&self.config);
        connector
            ...
            .and_then(|block| {
                tracker.get_all_smart_contract_tracking_data(&block.unwrap().hash.unwrap())
            })
    }
}
