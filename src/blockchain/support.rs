...
use project_generator::{DEFAULT_DATADIR_NAME, DEFAULT_DEV_PASSWORDS_DIR, DEFAULT_ENVIRONMENT};
    ...
    match client.parse() {
        Ok(SupportedBlockchainClients::Parity) => {
            ...
            if options.contains(&password_option) {
                ...
                if options[i].ends_with(&default_password_file.to_str().unwrap())
                    && !default_password_file.exists()
                {
...
pub fn default_options_from(
    client: SupportedBlockchainClients,
    vibranium_dir_path: &PathBuf,
) -> Vec<String> {
    match client {
        SupportedBlockchainClients::Parity => vec![
            "--config".to_string(),
            "dev".to_string(),
        ...
        SupportedBlockchainClients::Geth => vec![
            "--dev".to_string(),
            "--rpc".to_string(),
            "--ws".to_string(),
            "--wsorigins".to_string(),
        ...
        SupportedBlockchainClients::Ganache => vec![
