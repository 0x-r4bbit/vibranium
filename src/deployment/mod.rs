...
use config::{Config, SmartContractArg, SmartContractConfig};
...
impl<'a> Deployer<'a> {
    ...
    pub fn deploy(&self, options: DeployOptions) -> Result<DeployedContracts, DeploymentError> {
        ...
        let general_gas_price = deployment_config
            .gas_price
            .map(U256::from)
            .unwrap_or_else(|| {
                self.connector
        ...
        let general_gas_limit = deployment_config
            .gas_limit
            .map(U256::from)
            .unwrap_or_else(|| U256::from(DEFAULT_GAS_LIMIT));

        let confirmations = deployment_config
            .tx_confirmations
            .unwrap_or(DEFAULT_DEV_TX_CONFIRMATION_AMOUNT);
        let mut deployed_contracts = HashMap::new();

        let tracking_enabled = options
            .tracking_enabled
        ...
        let sorted_smart_contract_configs =
            sort_by_dependencies(&deployment_config.smart_contracts)?;

        for smart_contract_config in sorted_smart_contract_configs {
            if let Some(address) = &smart_contract_config.address {
                let address = Address::from_str(&address[2..]).map_err(|err| {
                    DeploymentError::InvalidAddress(
                ...
                info!(
                    ...
                    &smart_contract_config.name, &address
                );
                deployed_contracts.insert(
                    address,
                    (
                        smart_contract_config.name.clone(),
                        address,
                        "unknown".to_string(),
                        true,
                    ),
            ...
            if let Some((bin_path, abi_path)) =
                self.get_artifacts(&project_config.sources.artifacts, smart_contract_config)?
            {
                let bytecode = fs::read_to_string(&bin_path).unwrap();
                let abi = fs::read(abi_path).unwrap();

                let args = smart_contract_config
                    .args
                    .as_ref()
                    .unwrap_or(&vec![])
                    .iter()
                ...
                if tracking_enabled {
                    ...
                    let tracked_contract = self.tracker.get_smart_contract_tracking_data(
                        &block_hash,
                        &smart_contract_config.name,
                        &bytecode,
                        &args,
                    )?;
                    ...
                    if let Some(tracked_contract) = tracked_contract {
                        info!(
                            "{} is already deployed at {:?}",
                            &tracked_contract.name, &tracked_contract.address
                        );
                        deployed_contracts.insert(
                            tracked_contract.address,
                            (
                                tracked_contract.name,
                                tracked_contract.address,
                ...
                let pending_contract = builder
                    ...
                    .options(Options::with(|opts| {
                        opts.gas_price = smart_contract_config
                            .gas_price
                            .map(U256::from)
                            .or_else(|| Some(general_gas_price));
                        opts.gas = smart_contract_config
                            .gas_limit
                            .map(U256::from)
                            .or_else(|| Some(general_gas_limit));
                    }))
                    .execute(&bytecode, &*tokenized_args, accounts[0])
                    .map_err(|err| {
                        DeploymentError::InvalidConstructorArgs(
                            err,
                            smart_contract_config.name.to_owned(),
                        )
                    })?;

                let contract = pending_contract.wait().map_err(|err| {
                    DeploymentError::DeployContract(err, smart_contract_config.name.to_owned())
                })?;
                ...
                warn!(
                    "No bytecode or ABI found for Smart Contract '{}'",
                    &smart_contract_config.name
                );
            }
        }
    ...
    fn get_artifacts(
    ...
    ) -> Result<Option<(PathBuf, PathBuf)>, DeploymentError> {
        if config.bytecode_path.is_some() && config.abi_path.is_none() {
            ...
            Err(DeploymentError::MissingBytecodePath(
            ...
            let bytecode_path = self
                .config
                .project_path
                .join(&PathBuf::from(config.bytecode_path.as_ref().unwrap()));
            let abi_path = self
                .config
                .project_path
                .join(&PathBuf::from(config.abi_path.as_ref().unwrap()));
            info!(
                "Using pre-defined artifacts: {:?} and {:?}",
            ...
            let artifact_names: Vec<PathBuf> =
                artifacts_dir.map(|res| res.unwrap().path()).collect();
            let smart_contract_name = config.instance_of.as_ref().unwrap_or(&config.name);

            if let Some(artifact) = artifact_names.iter().find(|path| {
                path.to_string_lossy()
                    .to_string()
                    .contains(smart_contract_name)
            }) {
                let file_extension = &artifact.extension().unwrap().to_str().unwrap();

                if file_extension == &ARTIFACT_EXTENSION_BINARY
                    || file_extension == &ARTIFACT_EXTENSION_ABI
                {
                    let file_bin_path =
                        PathBuf::from(&artifact).with_extension(&ARTIFACT_EXTENSION_BINARY);
                    let file_abi_path =
                        PathBuf::from(&artifact).with_extension(&ARTIFACT_EXTENSION_ABI);

                    if file_extension == &ARTIFACT_EXTENSION_BINARY && !file_abi_path.exists() {
                        return Err(DeploymentError::MissingArtifact(
                            ARTIFACT_EXTENSION_ABI.to_string(),
                            file_bin_path.to_string_lossy().to_string(),
                        ));
                    } else if file_extension == &ARTIFACT_EXTENSION_ABI && !file_bin_path.exists() {
                        return Err(DeploymentError::MissingArtifact(
                            ARTIFACT_EXTENSION_BINARY.to_string(),
                            file_abi_path.to_string_lossy().to_string(),
                        ));
                    }
...
fn tokenize_args(
    ...
    deployed_contracts: &HashMap<Address, (String, Address, String, bool)>,
) -> Result<Vec<Token>, DeploymentError> {
    ...
    for arg in args {
        ...
        let value = if ParamType::Address == param_type {
            if arg.value.starts_with('$') {
                let contract = deployed_contracts
                    .values()
                    .find(|values| values.0 == arg.value[1..])
                    .unwrap();
        ...
        let token = LenientTokenizer::tokenize(&param_type, &value)
            .map_err(|err| DeploymentError::TokenizeParam(err, value.to_owned()))?;
...
fn sort_by_dependencies(
...
) -> Result<Vec<&SmartContractConfig>, DeploymentError> {
    let graph = DiGraphMap::<&str, ()>::from_edges(
        smart_contracts
            .iter()
            .filter(|contract| contract.args.is_some())
            .flat_map(|contract| {
                contract
                    .args
                    .as_ref()
    ...
    let sorted_names = toposort(&graph, None)
        .map_err(|err| DeploymentError::CyclicDependency(graph[err.node_id()].to_string()))?;
    ...
    for i in sorted_names.into_iter() {
        if smart_contract_map.get(graph[i]).is_none() {
            return Err(DeploymentError::MissingConfigForReference(
                graph[i].to_owned(),
            ));
        }
