...
impl fmt::Display for DeploymentTrackingError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            DeploymentTrackingError::DatabaseNotFound => {
                write!(f, "Couldn't find tracking database")
            }
            DeploymentTrackingError::Deserialization(error) => {
                write!(f, "Couldn't deserialize tracking data: {}", error)
            }
            DeploymentTrackingError::Serialization(error) => {
                write!(f, "Couldn't serialize tracking data: {}", error)
            }
            DeploymentTrackingError::Insertion(error) => write!(
                f,
                "Couldn't insert tracking data before writing to disc: {}",
                error
            ),
            DeploymentTrackingError::Read(error) => {
                write!(f, "Couldn't read tracking data: {}", error)
            }
            DeploymentTrackingError::Set(error) => {
                write!(f, "Couldn't set tracking data: {}", error)
            }
            DeploymentTrackingError::Delete(error) => {
                write!(f, "Couldn't delete tracking data: {}", error)
            }
        }
    }
}
