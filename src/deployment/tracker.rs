...
impl<'a> DeploymentTracker<'a> {
    ...
    pub fn track(
        ...
        args: &[String],
        address: Address,
    ) -> Result<(), DeploymentTrackingError> {
        ...
        match chain_tracking_data {
            None => tracking_data
                .insert(&query, new_tracking_data)
                .map_err(DeploymentTrackingError::Insertion)?,
            Some(_) => tracking_data
                .set(&query, new_tracking_data)
                .map_err(DeploymentTrackingError::Insertion)?,
        };
    ...
    pub fn get_smart_contract_tracking_data(
        &self,
        block_hash: &H256,
        name: &str,
        byte_code: &str,
        args: &[String],
    ) -> Result<Option<SmartContractTrackingDataEntry>, DeploymentTrackingError> {
        let block_hash = create_block_hash(&block_hash);
        let smart_contract_hash = create_smart_contract_hash(&name, &byte_code, &args);
        let tracking_data = self.try_from_tracking_file()?;
        let contract_data =
            tracking_data.read(&format!("{}.{}", &block_hash, &smart_contract_hash))?;

        if let Some(contract_data) = contract_data {
            Ok(Some(
                contract_data
                    .to_owned()
                    .try_into::<SmartContractTrackingDataEntry>()?,
            ))
        } else {
            Ok(None)
        }
    ...
    pub fn get_all_smart_contract_tracking_data(
        &self,
        block_hash: &H256,
    ) -> Result<Option<SmartContractTrackingData>, DeploymentTrackingError> {
        let block_hash = create_block_hash(&block_hash);
            ...
            Ok(tracking_data) => {
                ...
                if let Some(contract_data) = contract_data {
                    Ok(Some(
                        contract_data
                            .to_owned()
                            .try_into::<SmartContractTrackingData>()?,
                    ))
                } else {
                    Ok(None)
                }
    ...
    fn write(&self, toml: toml::Value) -> Result<(), DeploymentTrackingError> {
        ...
        tracking_file
            .write_all(tracking_data.as_bytes())
            .map_err(|err| DeploymentTrackingError::Other(err.to_string()))
    }
