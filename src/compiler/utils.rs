...
pub fn resolve_import(
...
) -> Result<ResolvedImport, error::CompilerError> {
    ...
    if is_node_module_import(&import, file_path, &project_path) {
        ...
        let normalized_path =
            destination_path.join(resolved_path.strip_prefix(project_path).unwrap());
        ...
        let normalized_path = destination_path.join(
            resolved_path
                .strip_prefix(project_path.join(INTERNAL_SOURCE_DIR))
                .unwrap(),
        );
        Ok((ImportType::Internal, resolved_path, normalized_path))
    } else {
        let canonicalized_import = import
            .canonicalize()
            .map_err(|_| error::CompilerError::ImportError(file_path.join(import)))?;
...
fn is_node_module_import<T: AsRef<Path>>(
    path: T,
    parent_path: &Path,
    project_path: &PathBuf,
) -> bool {
    if path.as_ref().is_absolute() {
        ...
        path.as_ref()
            .starts_with(project_path.join(NODE_MODULES_DIR))
            && path.as_ref().exists()
...
mod tests {
    ...
    mod resolve_import {
        ...
        fn it_should_resolve_internal_imports() -> Result<(), Box<std::error::Error>> {
            ...
            assert_eq!(
                resolved_import.2,
                destination_path.join(&import_path.strip_prefix("contracts").unwrap())
            );
        ...
        fn it_should_resolve_node_module_imports() -> Result<(), Box<std::error::Error>> {
            ...
            assert_eq!(
                resolved_import.1,
                absolute_node_modules_path.join(&import_path)
            );
            assert_eq!(
                resolved_import.2,
                destination_path.join("node_modules").join(&import_path)
            );
        ...
        fn it_should_resolve_external_imports() -> Result<(), Box<std::error::Error>> {
            ...
            let destination_path = project_path.join(".vibranium").join("contracts");
            let absolute_external_path = external_project_path
                .join("contracts")
                .join("something")
                .join("foo.sol");
            ...
            let resolved_import = resolve_import(
                &absolute_external_path.to_str().unwrap(),
            ...
            assert_eq!(
                resolved_import.2,
                destination_path.join(strip_absolute_prefix(&absolute_external_path))
            );
    ...
    mod is_node_module_import {
        ...
        fn it_should_be_true_with_implicit_import_syntax() -> Result<(), Box<std::error::Error>> {
            ...
            assert_eq!(
                is_node_module_import(&import_path, &project_path, &project_path),
                true
            );
        ...
        fn it_should_be_true_with_explicit_import_syntax() -> Result<(), Box<std::error::Error>> {
            ...
            assert_eq!(
                is_node_module_import(&relative_node_module_path, &project_path, &project_path),
                true
            );
    ...
    mod is_internal_import {
        ...
        #[test]
        fn it_should_be_true_if_absolute_import_is_within_project(
            ...
            assert_eq!(
                is_internal_import(&absolute_path, &project_path, &project_path),
                true
            );
        ...
        #[test]
        fn it_should_be_true_if_relative_import_is_within_project(
        ) -> Result<(), Box<std::error::Error>> {
            let (tmp_dir, project_path) = create_test_project()?;
            let import_path = PathBuf::from("contracts").join("something").join("foo.sol");
            let absolute_path = project_path.join(&import_path);
            ...
            assert_eq!(
                is_internal_import(&import_path, &project_path, &project_path),
                true
            );
