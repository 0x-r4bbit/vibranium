...
impl<'a> Compiler<'a> {
    ...
    pub fn normalize_imports(&self) -> Result<Vec<PathBuf>, error::CompilerError> {
        ...
        let mut unread = project_config
            ...
            .map(|pattern| {
                ...
                info!(
                    "Searching for source files with pattern: {:?}",
                    &full_pattern.display().to_string()
                );
                full_pattern
            })
            .flat_map(|path| {
                glob(&path.display().to_string())
                    .unwrap()
        ...
        while let Some(path) = unread.pop() {
            if let Ok(mut contents) = fs::read_to_string(&path) {
                let destination_path = utils::get_destination_path(
                    &path,
                ...
                for import in imports {
                    let resolved_import = utils::resolve_import(
                        &import,
                        &path.parent().unwrap(),
                        &self.config.project_path,
                        &destination_root,
    ...
    pub fn compile(&self, config: CompilerConfig) -> Result<Child, error::CompilerError> {
        ...
        // it into the correct format depending on plattform.
        let artifacts_path = PathBuf::from(&project_config.sources.artifacts)
            .components()
            .collect::<PathBuf>();
        ...
        let compiler = config
            .compiler
            .unwrap_or_else(|| match &project_config.compiler {
                Some(config) => config
                    .cmd
                    .clone()
                    .unwrap_or_else(|| SupportedCompilers::Solc.executable()),
                None => SupportedCompilers::Solc.executable(),
            });

        let mut compiler_options = match &config.compiler_options {
            Some(options) => match compiler.parse() {
            ...
            None => match project_config.compiler {
                Some(config) => config
                    .options
                    .unwrap_or_else(|| try_default_options_from(&compiler)),
        ...
        let input_files = if !config.smart_imports_enabled {
            ...
            match compiler.parse() {
                Ok(SupportedCompilers::Solc) => self
                    .normalize_imports()?
                    .iter()
                    .map(|path| path.to_string_lossy().to_string())
                    .collect::<Vec<String>>(),
                Ok(SupportedCompilers::SolcJs) => self
                    .normalize_imports()?
                    .iter()
                    .map(|path| path.to_string_lossy().to_string())
                    .collect::<Vec<String>>(),
                Err(_err) => self.find_input_files(&project_config.sources.smart_contracts),
            }
        };
        ...
        info!(
            "Compiling project using command: {} {} {}",
            &shell,
            &shell_opt,
            compiler_options.join(" ")
        );
        ...
        Command::new(shell)
            ...
            .map_err(|err| match err.kind() {
                std::io::ErrorKind::NotFound => {
                    error::CompilerError::ExecutableNotFound(err, shell.to_owned())
                }
